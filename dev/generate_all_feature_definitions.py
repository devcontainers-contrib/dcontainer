...
def generate_all_Feature_definitions(
    ...
    for feature_name in os.listdir(feature_definitions_dir):
        ...
        if remove_old_content and os.path.exists(os.path.join("test", feature_name)):
            shutil.rmtree(os.path.join("test", feature_name))
        if remove_old_content and os.path.exists(os.path.join("src", feature_name)):
            shutil.rmtree(os.path.join("src", feature_name))

        OCIFeatureGenerator.generate(
            Path(feature_definition_file),
